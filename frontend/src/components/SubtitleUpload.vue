<template>
  <div class="subtitle-upload">
    <div v-if="!project.video_filename" class="upload-zone" @dragover.prevent @drop.prevent="onDrop">
      ...
      <p class="upload-hint">支持 mp4, avi, mkv, mov, flv, webm 等格式</p>
    ...
    <div v-else class="file-info">
      ...
      <div class="file-actions">
        <el-upload :show-file-list="false" :before-upload="beforeUpload" :http-request="handleUpload" accept="video/*">
...
import { uploadSubtitleVideo, importSubtitleSrt } from '../api/subtitle'
...
function beforeUpload(file) {
  ...
  const allowed = ['mp4', 'avi', 'mkv', 'mov', 'wmv', 'flv', 'webm', 'm4v', 'mpeg', 'mpg', 'ts']
...
async function handleUpload(options) {
  ...
  try {
    ...
    await uploadSubtitleVideo(props.project.id, formData, (e) => {
